...
float4 main(PS_IN i) : SV_Target
{
	...
	for (int t1 = -1; t1 <= 1; t1 = t1 + 1) {
		t0.w = t0.w + shadowMap.SampleCmpLevelZero(shadowSamp, float2((float)t1 * bias.z + t0.x, t0.y - bias.w), t0.z).x;
	}
	float t2 = t0.w;
	...
	for (int t5 = -1; t5 <= 1; t5 = t5 + 1) {
		t4 = t4 + shadowMap.SampleCmpLevelZero(shadowSamp, float2((float)t5 * bias.z + t0.x, t0.y + bias.w), t0.z).x;
	}
