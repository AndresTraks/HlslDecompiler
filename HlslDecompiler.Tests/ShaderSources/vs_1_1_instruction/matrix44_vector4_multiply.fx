float4x4 matrix_4x4;

struct VS_OUT
{
	float4 position : POSITION;
	float4 texcoord4 : TEXCOORD4;
	float4 texcoord1 : TEXCOORD1;
	float4 texcoord2 : TEXCOORD2;
	float4 texcoord3 : TEXCOORD3;
};

VS_OUT main(float4 position : POSITION)
{
	VS_OUT o;

	float4 r0;
	float4 r1;
	r0 = position.y * transpose(matrix_4x4)[1];
	r0 = transpose(matrix_4x4)[0] * position.x + r0;
	r0 = transpose(matrix_4x4)[2] * position.z + r0;
	o.position = transpose(matrix_4x4)[3] * position.w + r0;
	o.texcoord4 = r0 + transpose(matrix_4x4)[3];
	r0 = position.x * transpose(matrix_4x4)[1];
	r0 = transpose(matrix_4x4)[0] * position.y + r0;
	r0 = transpose(matrix_4x4)[2] * position.z + r0;
	o.texcoord1 = transpose(matrix_4x4)[3] * position.w + r0;
	r0 = max(-position.yxzw, position.yxzw);
	r1 = r0.y * transpose(matrix_4x4)[1];
	r1 = transpose(matrix_4x4)[0] * r0.x + r1;
	r1 = transpose(matrix_4x4)[2] * r0.z + r1;
	o.texcoord2 = transpose(matrix_4x4)[3] * r0.w + r1;
	r0 = position * float4(5, 2, 3, 4);
	r1 = r0.y * transpose(matrix_4x4)[1];
	r1 = transpose(matrix_4x4)[0] * r0.x + r1;
	r1 = transpose(matrix_4x4)[2] * r0.z + r1;
	o.texcoord3 = transpose(matrix_4x4)[3] * r0.w + r1;

	return o;
}
