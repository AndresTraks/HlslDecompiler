...
VS_OUT main(float4 position : POSITION)
{
	...
	float4 r0;
	r0 = transpose(matrix_3x2)[0].xyxy * position.xyyx;
	o.position.xz = r0.yw + r0.xz;
	r0 = transpose(matrix_3x2)[1].xyxy * position.xyyx;
	o.position.yw = r0.yw + r0.xz;
	r0.xy = transpose(matrix_3x2)[0].xy * abs(position.yx);
	o.position1.x = r0.y + r0.x;
	r0.xy = transpose(matrix_3x2)[1].xy * abs(position.yx);
